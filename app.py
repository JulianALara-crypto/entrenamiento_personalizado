...
URL_API = "https://script.google.com/macros/s/AKfycbwlxInt5LaaJ1B4wiBjiw5tlPyIjgEk_G0ButQFxWlmhJfGImZOYNCXFhPtlud5LfSU/exec"
...
if icono_pestana is not None:
    ...
    with col_l2:
        st.image(icono_pestana, width="stretch")
...
def calcular_metricas(peso, estatura_cm, edad, sexo, cuello, cintura, cadera, meta):
    ...
    try:
        if sexo == "Masculino":
            ...
            if valor_log <= 0:
                valor_log = 1.0
            densidad = (1.0324 - 0.19077 * math.log10(valor_log) + 0.15456 * math.log10(estatura_cm))
            pct_grasa = (495 / densidad) - 450
        else:
            valor_log = cintura + cadera - cuello
            if valor_log <= 0:
                valor_log = 1.0
            densidad = (1.29579 - 0.35004 * math.log10(valor_log) + 0.22100 * math.log10(estatura_cm))
            pct_grasa = (495 / densidad) - 450
    ...
    return (float(round(imc, 2)), float(round(pct_grasa, 2)), int(round(calorias)), int(edad_metabolica))
...
def link_whatsapp(num_celular, nombre_cliente, mensaje=""):
    ...
    if not mensaje:
        mensaje = (f"💪 ¡Hola {nombre_cliente}! Te saludamos de tu plan de Entrenamiento Personalizado. "
                   "¡Queremos revisar cómo van tus avances!")
...
def cargar_bd():
    try:
        ...
        if len(pagos_raw) > 1:
            ...
            df_p = pd.DataFrame(columns=["id_pago", "cedula", "fecha_pago", "valor", "concepto", "valor_mensualidad"])
        ...
        if len(clases_raw) > 1:
            ...
            df_c = pd.DataFrame(columns=["id_clase", "cedula", "nombre_completo", "fecha_clase", "tipo_plan", "periodo", "estado"])
        ...
        columnas_numericas_medidas = ["edad", "peso_kg", "estatura_cm", "cuello_cm", "hombros_cm", "bicep_der_cm", "bicep_izq_cm", "pecho_cm", "cintura_cm", "cadera_cm", "pierna_der_cm", "pierna_izq_cm", "gemelo_der_cm", "gemelo_izq_cm", "imc", "porcentaje_grasa", "calorias_objetivo", "edad_metabolica"]
...
def mostrar_graficos_evolucion(df_filtrado):
    ...
    columnas_num = ["peso_kg", "porcentaje_grasa", "cintura_cm", "pecho_cm", "cadera_cm", "bicep_der_cm", "bicep_izq_cm"]
    
    for col in columnas_num:
        if col in df_graficos.columns:
            df_graficos[col] = pd.to_numeric(df_graficos[col], errors="coerce")

    df_graficos["fecha_dt"] = pd.to_datetime(df_graficos["fecha_evaluacion"], format="%d-%m-%Y", errors="coerce")
    
    if df_graficos["fecha_dt"].isna().any():
        df_graficos["fecha_dt"] = pd.to_datetime(df_graficos["fecha_evaluacion"], errors="coerce")

    df_graficos = df_graficos.dropna(subset=["fecha_dt"]).sort_values(by="fecha_dt")
    df_graficos["Fecha"] = df_graficos["fecha_dt"].dt.strftime("%d-%m-%Y")

    st.markdown("### 📈 Gráficas de Evolución Temporal")
    tab1, tab2, tab3 = st.tabs(["⚖️ Peso y Composición", "📏 Perímetros Principales", "💪 Extremidades"])
...
def obtener_resumen_clases(df_clases, cedula):
    ...
    registros = df_clases[df_clases["cedula"].apply(normalizar_cedula) == cedula].copy()

    if registros.empty:
        return resultado

    registros["_orden"] = range(len(registros))
    for columna, valor in {"fecha_clase": "", "tipo_plan": "", "periodo": pd.NA, "estado": ""}.items():
    ...
    registros["_es_config"] = ((fechas_texto == "") & (estados_texto.eq("configuración del plan") | estados_texto.eq("configuracion del plan") | estados_texto.eq("")))
    registros["_es_tomada"] = ((fechas_texto != "") & (estados_texto.eq("tomada") | estados_texto.eq("clase tomada") | estados_texto.eq("")))
    ...
    if indice_config is not None:
        registros_tomados = registros[(registros["_orden"] > indice_config) & registros["_es_tomada"]].copy()
    else:
    ...
    resultado["registros"] = registros_tomados.drop(columns=["_orden", "_es_config", "_es_tomada"], errors="ignore")
    return resultado
...
def mostrar_resumen_clases(df_clases, cedula, titulo="🏋️ Clases Personalizadas"):
    ...
    st.markdown(
        f"""
        <div style="text-align:center;font-size:18px;margin-top:-10px;margin-bottom:15px;">
        <strong>{resumen['porcentaje']:.1f}% del plan utilizado</strong>
        </div>
        """,
        unsafe_allow_html=True
    )
...
if not st.session_state["autenticado"]:
    ...
    with col2:
        ...
        with st.form("form_registro"):
            ...
            reg_whatsapp = st.text_input("Número de Whatsapp (10 dígitos):", placeholder="310.......").strip()
                ...
                if not reg_cedula or not reg_nombre or not reg_pass:
                    ...
                    try:
                        respuesta_registro = requests.post(
                            URL_API,
                            json={"action": "registrar_usuario", "row": nueva_fila},
                            timeout=30,
                        )
...
# ============================================================
    ...
    if st.session_state["rol"] == "Cliente":
        opcion = st.sidebar.radio("MENÚ", ["📏 Registrar Medidas Hoy", "📊 Ver Mi Progreso", "🏋️ Mis Clases"])
            ...
            with st.form("form_medidas_cliente"):
                ...
                if st.form_submit_button("Guardar Evaluación"):
                    try:
                        ...
                        if imc < 10 or imc > 60:
                            st.error(f"⚠️ El IMC calculado ({imc}) está fuera de un rango razonable. Revisa peso y estatura.")
                            st.stop()
                        ...
                        fila_medidas = [
                            str(id_reg), str(fecha_hoy), str(st.session_state["cedula"]), int(edad), str(sexo), str(meta),
                            float(peso), float(estatura), float(cuello), float(hombros), float(bicep_der), float(bicep_izq),
                            float(pecho), float(cintura), float(cadera), float(pierna_der), float(pierna_izq), float(gemelo_der),
                            float(gemelo_izq), float(imc), float(grasa), int(cals), int(edad_bio)
                        ]

                        respuesta_medidas = requests.post(URL_API, json={"action": "guardar_medidas", "row": fila_medidas}, timeout=30)
                        ...
                        if resultado_api.get("status") == "error":
                            st.error("❌ Google Apps Script reportó un error: " + str(resultado_api.get("message", "Error desconocido")))
                            st.stop()
        ...
        # VER PROGRESO
            ...
            if not mis_registros.empty:
                ...
                mis_registros["_fecha_dt"] = pd.to_datetime(mis_registros["fecha_evaluacion"], format="%d-%m-%Y", errors="coerce")
                ...
                if len(mis_registros) >= 2:
                    ...
                    st.info("Aún no tienes suficientes registros para mostrar variaciones.")
            else:
        ...
        # MIS CLASES
            ...
            if not registros_clases.empty:
                ...
                columnas_tabla = [c for c in ["fecha_clase", "tipo_plan", "periodo", "estado"] if c in tabla.columns]
                tabla = tabla[columnas_tabla].rename(columns={
                    "fecha_clase": "Fecha de Clase", "tipo_plan": "Plan",
    ...
    # ========================================================
        ...
        if not df_usuarios.empty:
            ...
            if not clientes.empty:
                opcion_admin = st.sidebar.radio("MENÚ ADMINISTRADOR", ["👤 Gestión de Clientes", "🏋️ Control de Clases"])
                    ...
                    if cedula_sel:
                        ...
                        if not cliente_encontrado.empty:
                            ...
                            if not pagos_cliente.empty:
                                pagos_cliente["_fecha_dt"] = pagos_cliente["fecha_pago"].apply(parsear_fecha) if "fecha_pago" in pagos_cliente.columns else pd.NaT
                                pagos_mes_actual = pagos_cliente[pagos_cliente["_fecha_dt"].notna() & (pagos_cliente["_fecha_dt"].dt.year == hoy.year) & (pagos_cliente["_fecha_dt"].dt.month == hoy.month)].copy()
                            else:
                            ...
                            if not pagos_cliente.empty and "valor_mensualidad" in pagos_cliente.columns:
                                mensualidades_validas = pd.to_numeric(pagos_cliente["valor_mensualidad"], errors="coerce").dropna()
                            ...
                            if not pagos_mes_actual.empty and "valor" in pagos_mes_actual.columns:
                                total_pagado = float(pd.to_numeric(pagos_mes_actual["valor"], errors="coerce").fillna(0).sum())
                            ...
                            st.caption(f"📅 Estado de pagos correspondiente a {hoy.strftime('%B').capitalize()} de {hoy.year}.")

                            with st.expander("➕ Registrar nuevo pago / abono", expanded=saldo_actual > 0):
                                with st.form(f"form_pago_{id_cliente}"):
                                    valor_mensualidad = st.number_input("Valor de la mensualidad ($):", min_value=1.0, value=float(valor_mensualidad_actual), step=5000.0, format="%.0f")
                                    saldo_para_nuevo_pago = max(float(valor_mensualidad) - total_pagado, 0.0)

                                    if total_pagado > 0:
                                        st.caption(f"Pagado este mes: ${saldo_para_nuevo_pago:,.0f}")

                                    valor_pago = st.number_input("Valor del pago / abono ($):", min_value=1.0, value=float(saldo_para_nuevo_pago if saldo_para_nuevo_pago > 0 else 1.0), step=5000.0, format="%.0f")
                                    concepto_pago = st.text_input("Concepto:", value="Abono mensualidad").strip()
                                    ...
                                    if st.form_submit_button("💾 Registrar Pago", use_container_width=True):
                                        try:
                                            ...
                                            if nuevo_total > valor_mensualidad + 0.001:
                                                st.error(f"❌ El abono supera el saldo pendiente. Saldo disponible: ${saldo_para_nuevo_pago:,.0f}.")
                                                st.stop()

                                            id_pago = f"{id_cliente}_{datetime.today().strftime('%Y%m%d%H%M%S%f')}"
                                            fecha_pago = datetime.today().strftime("%d-%m-%Y")

                                            fila_pago = [str(id_pago), str(id_cliente), str(fecha_pago), float(valor_pago), str(concepto_pago), float(valor_mensualidad)]
                                            respuesta_pago = requests.post(URL_API, json={"action": "guardar_pago", "row": fila_pago}, timeout=30)
                                            respuesta_pago.raise_for_status()
                                            
                                            st.cache_data.clear()
                                            nuevo_saldo = max(valor_mensualidad - nuevo_total, 0.0)

                                            if nuevo_saldo <= 0.001:
                                                st.success("✅ Pago registrado. La mensualidad de este mes quedó completamente pagada.")
                                            else:
                                                st.success(f"✅ Abono registrado correctamente. Saldo pendiente: ${nuevo_saldo:,.0f}.")
                                            st.rerun()
                                        except Exception as e:
                            ...
                            if not pagos_cliente.empty:
                                ...
                                if "_fecha_dt" not in pagos_mostrar.columns:
                                    pagos_mostrar["_fecha_dt"] = pagos_mostrar["fecha_pago"].apply(parsear_fecha) if "fecha_pago" in pagos_mostrar.columns else pd.NaT

                                pagos_mostrar = pagos_mostrar.sort_values(by="_fecha_dt", ascending=False).drop(columns=["_fecha_dt"], errors="ignore")
                                columnas_pago_mostrar = [c for c in ["fecha_pago", "valor", "concepto", "valor_mensualidad"] if c in pagos_mostrar.columns]
                                ...
                                tabla_pagos = tabla_pagos.rename(columns={"fecha_pago": "Fecha", "valor": "Pago", "concepto": "Concepto", "valor_mensualidad": "Mensualidad"})
                                st.dataframe(tabla_pagos, use_container_width=True, hide_index=True)
                            else:
                                st.info("Este cliente todavía no tiene pagos registrados.")

                            st.markdown("---")
                            mostrar_resumen_clases(df_clases, id_cliente, "🏋️ Historial de Clases")

                            resumen_cliente_clases = obtener_resumen_clases(df_clases, id_cliente)
                            if not resumen_cliente_clases["registros"].empty:
                                clases_mostrar = resumen_cliente_clases["registros"].copy()
                                columnas_clases = [c for c in ["fecha_clase", "tipo_plan", "periodo", "estado"] if c in clases_mostrar.columns]
                                tabla_clases = clases_mostrar[columnas_clases].copy()
                                tabla_clases = tabla_clases.rename(columns={"fecha_clase": "Fecha Clase", "tipo_plan": "Plan", "periodo": "Periodo", "estado": "Estado"})
                                st.dataframe(tabla_clases.astype(str), use_container_width=True, hide_index=True)
                            ...
                            if not df_historial.empty:
                                ...
                                if not h_cliente.empty:
                                    ...
                                    h_cliente["_fecha_dt"] = pd.to_datetime(h_cliente["fecha_evaluacion"], format="%d-%m-%Y", errors="coerce")
                                    h_cliente_ord = h_cliente.sort_values(by="_fecha_dt", ascending=False).drop(columns=["_fecha_dt"])
                                    
                                    # COMPLETAR EL DATAFRAME FINAL TRUNCADO
                                    st.dataframe(h_cliente_ord.astype(str), use_container_width=True)
                                else:
                                    st.info("No hay registros de evolución para este cliente.")
                            else:
                                st.info("No hay registros de evolución en la base de datos general.")
